...
function sampleBitbucketIssueWithoutLabelPayload() {
  return {
    event: 'issue:updated',
    actor: { username: 'testuser', uuid: '{abc-123}' },
    repository: {
...
describe('createBitbucketWebhooks', () => {
  ...
  it("posts a 'working on it' comment before dispatching", async () => {
    const handler = createBitbucketWebhooks(mockEnqueue);
    ...
    expect(mockCreateComment).toHaveBeenCalledWith(
      'owner/test-repo',
      42,
      expect.stringContaining('working on this issue'),
    );
  });

  it('does NOT enqueue when the issue lacks the trigger label', async () => {
    const handler = createBitbucketWebhooks(mockEnqueue);

    const rawPayload = JSON.stringify(sampleBitbucketIssueWithoutLabelPayload());
    const expected = crypto.createHmac('sha256', 'test-secret').update(rawPayload, 'utf8').digest('hex');
    const signature = `sha256=${expected}`;
  ...
  it('still enqueues when the status comment fails (best-effort)', async () => {
