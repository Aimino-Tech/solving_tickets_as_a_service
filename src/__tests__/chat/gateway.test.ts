...
function inbound(overrides: Partial<Parameters<ChatGateway['route']>[0]> = {}) {
  return {
    threadTs: 't1', channelId: 'c1', userId: 'u1', text: 'hello', ts: 'now',
    ...overrides,
  };
}
...
describe('chat gateway (AIM-4442)', () => {
  ...
  it('routes to a registered pod and reports delivered', async () => {
    ...
    const sent: unknown[] = [];
    gw.registerPod('u1', {
      userId: 'u1', sessionId: 's1', lastHeartbeat: Date.now(),
  ...
  it('prunes stale pods after the idle timeout', () => {
    ...
    gw.registerPod('u1', {
      userId: 'u1', sessionId: 's1', lastHeartbeat: 3000,
  ...
  it('shutdown notifies all pods', () => {
    ...
    gw.registerPod('u1', {
      userId: 'u1', sessionId: 's1', lastHeartbeat: Date.now(),
      send: (msg) => sent.push(msg), close: () => undefined,
    });
    gw.shutdown();
