...
function applyFilters(incidents: NormalizedIncident[], query: Record<string, string | undefined>): NormalizedIncident[] {
  ...
  if (q) {
    filtered = filtered.filter(
      (i) => i.title.toLowerCase().includes(q) || i.service.toLowerCase().includes(q) || i.fingerprint.toLowerCase().includes(q),
    );
  }
  return filtered;
}

function attachCatalog(incidents: NormalizedIncident[], catalog: Array<{ name: string; repos: string[]; purpose: string | null; runbook: string | null }>) {
...
router.get('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const watcherPromise = Promise.all([notifyIncidentResolutions(incidents), persistIncidentStates(incidents)]).catch((err) =>
      log.warn({ err: String(err) }, 'Incident watcher failed'),
    );
...
router.put('/service-catalog/:id', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updated = await incidentServiceCatalogRepository.update(id, {
      ...
      providers: body.providers !== undefined ? (Array.isArray(body.providers) ? body.providers.map(String) : []) : undefined,
    });
    res.json(updated);
  } catch (err) {
