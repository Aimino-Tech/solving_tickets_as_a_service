...
/**
 * GET /admin/webhooks/health — Webhook health monitoring dashboard.
 *
 * Returns current status counts, failure rate, and recommendations.
 * The failure rate alert triggers when > 5% of webhooks have failed
 * in the recent processing window.
 */
router.get('/health', async (_req: Request, res: Response) => {
  try {
    ...
    const grandTotal = totalReceived + totalProcessed + totalFailed + totalDead + totalProcessing;

    // Failure rate: failed+dead vs total processed/failed
    ...
    const alertTriggered = failureRate > 5;
    ...
    res.json({
      ...
      failureRate: {
        rate: Math.round(failureRate * 100) / 100,
        threshold: 5,
        alertTriggered,
        message: alertTriggered
          ? `Failure rate ${failureRate.toFixed(1)}% exceeds 5% threshold — review dead letter queue`
          : 'Failure rate within acceptable range',
      },
      timestamp: new Date().toISOString(),
    });
  } catch (err) {
    log.error({ err: String(err) }, 'Failed to get webhook health status');
    res.status(500).json({ error: 'Failed to get webhook health status' });
  }
});
