...
export class JiraTracker implements Tracker {
  ...
  async postComment(ticketId: string, body: string): Promise<void> {
    const result = await this.api<{ id: string }>(
      `/issue/${encodeURIComponent(ticketId)}/comment`,
      {
        method: "POST",
  ...
  async createLink(ticketId: string, url: string, title: string): Promise<void> {
    try {
      ...
      log.warn(
        ...
        "Failed to create remote link on Jira issue",
      );
    }
  }
}
...
function extractJiraDescription(
  desc: JiraIssueFields["description"],
): string | null {
  if (!desc) return null;
...
function parseJiraPriority(
  priority: { id: string; name: string } | null | undefined,
): number {
...
export async function handleJiraWebhook(
  ...
  log.info(
    { ticketId, event, summary: body.issue.fields?.summary },
