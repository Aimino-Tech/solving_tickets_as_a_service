...
/**
 * Simple admin key check via `x-admin-key` header.
 * In production, this should be replaced with a proper admin auth gateway.
 */
function requireAdmin(req: Request, res: Response): boolean {
  const adminKey = req.headers['x-admin-key'];
  // Compare against env var or default to a known dev key
  const expectedKey = process.env.ADMIN_API_KEY || 'stas-admin-dev-key';
