...
export async function gateRealityCheck(
  ...
  if (!diff) return { gate: 'reality-check', passed: true, reason: 'No diff to check' };
  ...
  while ((m = fileRe.exec(diff)) !== null) {
    if (m[1].startsWith('.')) referencedPaths.add(m[1]);
  }
...
export async function gateCompileCheck(sandbox: SandboxExecutor): Promise<GateResult> {
  try {
    ...
    return { gate: 'compile-check', passed: false, reason: `Compilation check error: ${String(err)}` };
  }
}

export async function gateTestCheck(sandbox: SandboxExecutor, diff: string): Promise<GateResult> {
  if (!diff) return { gate: 'test-check', passed: true, reason: 'No diff to check' };

  const hasTestChanges = /(?:\+|\-)\s*.*(?:describe|it|test)\s*\(/g.test(diff);
  if (!hasTestChanges) return { gate: 'test-check', passed: true, reason: 'No test changes in diff' };
  ...
  for (const pattern of vacuousPatterns) {
    if (pattern.test(diff))
      return { gate: 'test-check', passed: false, reason: 'Vacuous assertion detected', details: `Pattern matched: ${pattern}` };
  }
  ...
  if (nonVacuousAssertions.length === 0)
    return { gate: 'test-check', passed: false, reason: 'No assertions found in added test code' };
...
export async function gateHallucinationCheck(sandbox: SandboxExecutor, diff: string): Promise<GateResult> {
  if (!diff) return { gate: 'hallucination-check', passed: true, reason: 'No diff to check' };
  ...
  if (newImports.size === 0) return { gate: 'hallucination-check', passed: true, reason: 'No new external imports detected' };
  ...
  try {
    ...
    return { gate: 'hallucination-check', passed: false, reason: `Hallucination check error: ${String(err)}` };
  }
}
...
export async function runQualityGates(
  ...
  for (const gateFn of gates) {
    try {
      ...
      results.push({ gate: 'unknown', passed: false, reason: `Gate threw an error: ${String(err)}` });
      allPassed = false;
    }
  }
