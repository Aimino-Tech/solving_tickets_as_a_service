...
export class BullMQQueueAdapter implements QueueAdapter {
	...
	async enqueue(
		...
		try {
			...
			log.error(
				{ err: String(err), queue: this.cfg.name },
	...
	async startConsumer(
		handler: (data: IssueJobData) => Promise<void>,
	): Promise<void> {
		...
		this.worker.on("failed", (job, err) => {
			log.error(
				{ jobId: job?.id, err: String(err), queue: this.cfg.name },
				"BullMQ job failed",
			);
		});
		...
		log.info(
			{ queue: this.cfg.name, concurrency: config.queue.workerConcurrency },
			"BullMQ consumer started",
		);
	}
