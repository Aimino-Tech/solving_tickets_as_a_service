...
export interface MemoryExtractor {
  (prev: AgentMemory, userText: string, assistantText: string): MemoryDelta;
}
...
const FACT_PATTERNS: { key: string; re: RegExp }[] = [
  ...
  { key: 'project', re: /(?:the project is|we(?:['’]?re| are) (?:working on|building|using|on))\s+(?:a |an |the )?([A-Za-z0-9_\-./]+)/i },
  { key: 'language', re: /(?:language|stack)(?:\s+is|\s+of|\s+we use|\s+we['’]?re using)?\s+([A-Za-z#+]+)/i },
  { key: 'framework', re: /(?:framework|using|built with)\s+([A-Za-z][A-Za-z0-9.\- ]{1,30}?)(?:\s+for|\s+to|\s+and|,|\.|$)/i },
