...
const STOPWORDS = new Set([
  ...
  'when', 'what', 'will', 'should', 'would', 'could', 'there', 'their',
  'about', 'after', 'before', 'because', 'being', 'into', 'over', 'then',
  'also', 'just', 'does', 'still', 'make', 'made',
]);
 ...
 */
export function findReferencedFacts(userText: string, facts: Map<string, string>): Array<{ key: string; value: string }> {
...
export class OpenCodeExecutor implements AgentExecutor {
  ...
  async run(input: AgentInput): Promise<AgentOutput> {
    ...
    const prompt = [
      ...
      input.recentTranscript.length > 0 ? `[Recent Conversation History]\n${input.recentTranscript.map((t) => `${t.role}: ${t.text}`).join('\n')}` : '',
      `[User] ${input.userText}`,
    ]
      .filter(Boolean)
      .join('\n\n');
