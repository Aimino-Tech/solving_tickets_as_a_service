    ...
    if (Array.isArray(entries)) {
      for (const entry of entries) {
        if (typeof entry === 'string') {
          ...
          if (prUrl) {
            prs.push({ repo: String(e.repo ?? repos[0] ?? 'unknown'), prUrl, status: e.status ? String(e.status) : undefined });
          }
        }
      }
    }
...
export function computeStats(incidents: NormalizedIncident[]): IncidentStats {
  ...
  const mttrSeconds =
    mttrDurations.length > 0
      ? Math.round(mttrDurations.reduce((a, b) => a + b, 0) / mttrDurations.length)
      : null;
