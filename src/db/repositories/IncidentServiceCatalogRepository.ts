...
export class IncidentServiceCatalogRepository {
  ...
  async update(id: number, data: Partial<NewServiceCatalogEntry>): Promise<ServiceCatalogEntry | undefined> {
    ...
    if (data.repos !== undefined) { sets.push(`repos = $${idx++}`); values.push(data.repos); }
    if (data.purpose !== undefined) { sets.push(`purpose = $${idx++}`); values.push(data.purpose); }
    if (data.runbook !== undefined) { sets.push(`runbook = $${idx++}`); values.push(data.runbook); }
    if (data.providers !== undefined) { sets.push(`providers = $${idx++}`); values.push(data.providers); }
  ...
  async delete(id: number): Promise<boolean> {
    const result = await queryWithRetry<{ id: number }>('DELETE FROM incident_service_catalog WHERE id = $1 RETURNING id', [id]);
