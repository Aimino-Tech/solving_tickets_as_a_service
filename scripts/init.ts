#!/usr/bin/env tsx
/**
 * init.ts — STAS interactive setup wizard
 *
 * Guides users through configuring STAS for their repository.
...
function printBanner(): void {
  ...
  ███████  ████████  █████   ██████
  ██         ██    ██   ██  ██   ██
  ███████    ██    ███████  ██████       ${bold("Solving Tickets As A Service")}
       ██    ██    ██   ██  ██   ██
  ███████    ██    ██   ██  ██   ██      ${dim("Label an issue. Get a PR.")}

  ${bold("Interactive Setup Wizard")}
`)}`);
  console.log(dim("This wizard will guide you through configuring STAS for your repository."));
...
function generateEnvFile(env: EnvVars): string {
  ...
  h("=============================================================================");
  h("STAS — Solving Tickets As A Service");
  h(`Generated by npx stas-init on ${new Date().toISOString()}`);
  h("=============================================================================");
  lines.push("");
  ...
  // Bot settings
  h("=== Bot Settings ===");
  lines.push(`STAS_LABEL=${env.STAS_LABEL}`);
...
async function main(): Promise<void> {
  ...
  printSection(
    "Redis Configuration",
    "STAS uses Redis for the job queue. Default: redis://localhost:6379",
  );
  ...
  printSection(
    "Bot Settings",
    "Configure how STAS behaves on your repository.",
  );

  env.STAS_LABEL = await ask("Trigger label", {
    default: "stas:fix",
  });
  printValue("STAS_LABEL", env.STAS_LABEL);
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (!overwrite) {
      console.log(yellow("  ⚠ Keeping existing .env. Writing to .env.stas-init instead."));
      fs.writeFileSync(".env.stas-init", envContent, "utf-8");
      console.log(green("  ✓ Wrote .env.stas-init (review and merge manually)"));
    } else {
  ...
  if (startDev) {
    console.log(dim("\nStarting STAS in development mode...\n"));
...
`)}`);
  ...
  console.log(`  3. ${bold("Start STAS:")}      ${dim("npm run dev")}`);
  console.log(`  4. ${bold("Configure webhook:")} Point your GitHub App to http://your-server:${env.PORT || "3000"}/webhook`);
  console.log(`  5. ${bold("Test it:")}         Label any issue with "${env.STAS_LABEL}"`);
  console.log();
