#!/usr/bin/env python3
"""Post reply via Chrome CDP - simplified version."""
import json
import time
import websocket
...
if result:
    ...
    if data.get('found'):
        ...
        # Type reply
        reply_text = "This reframing is spot on. MCP as governance layer rather than just integration protocol changes how you architect agent systems. We found the key is mapping tool scopes to agent permissions."
        print(f"Typing reply ({len(reply_text)} chars)...", flush=True)
        ...
        if result:
            ...
            if data.get('found'):
                ...
                print(f"Clicking post at ({x}, {y})...", flush=True)
                cdp('Input.dispatchMouseEvent', {'type': 'mousePressed', 'x': x, 'y': y, 'button': 'left', 'clickCount': 1})
                cdp('Input.dispatchMouseEvent', {'type': 'mouseReleased', 'x': x, 'y': y, 'button': 'left', 'clickCount': 1})
                time.sleep(5)
