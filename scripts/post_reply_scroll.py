#!/usr/bin/env python3
"""Post reply with scroll to button."""
import json
import time
import websocket
...
# Scroll to make reply button visible
print("Scrolling to reply button...", flush=True)
...
print("Looking for reply button...", flush=True)
        ...
        result = cdp('Runtime.evaluate', {'expression': '''
            (function() {
                ...
                return JSON.stringify({
                    ...
                    dialogText: dialog ? dialog.innerText.substring(0, 100) : ''
                });
            })()
        '''})
        
        if result:
            dialog_info = json.loads(result.get('result', {}).get('result', {}).get('value', '{}'))
            print(f"Dialog info: {dialog_info}", flush=True)
