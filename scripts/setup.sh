#!/usr/bin/env bash
# =============================================================================
# scripts/setup.sh — SYNTARO One-Command Development Environment Setup
#
# Usage:
#   npm run setup
...
echo "ready to develop SYNTARO. Press Ctrl+C at any time to abort.${NC}"
echo ""
...
setup_env() {
  ...
  if [ -f ".env.example" ]; then
    ...
    cat > ".env" << 'ENVEOF'
# SYNTARO — Development Environment (auto-generated by npm run setup)
RUN_MODE=both
PORT=3000
  ...
  if ! docker info &>/dev/null 2>&1; then
    ...
    if [ "$REDIS_RUNNING" = "true" ]; then
      ...
      if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "syntaro-redis"; then
        info "syntaro-redis container already running"
      else
        if confirm "Start Docker services (Redis, RabbitMQ)?" "y"; then
          ...
          dim "  RabbitMQ:  amqp://guest:guest@localhost:5672/syntaro"
        else
...
echo -e "${GREEN}"
echo "  ┌──────────────────────────────────────────────────────┐"
echo "  │  ${BOLD}SYNTARO Development Environment is Ready${NC}${GREEN}             │"
echo "  └──────────────────────────────────────────────────────┘"
echo -e "${NC}"
