...
import { github, onboarding, request, type WizardProgress } from '@/api/client';

const FALLBACK_GITHUB_APP_URL = 'https://github.com/apps/syntaro-bot/installations/new';
...
export default function GitHubInstall({ onComplete, onSkip }: Props) {
  const [installing, setInstalling] = useState(false);
  const [githubToken, setGithubToken] = useState<string | null>(null);
  const [githubAppUrl, setGithubAppUrl] = useState<string>(FALLBACK_GITHUB_APP_URL);
  ...
  useEffect(() => {
    onboarding
      .getStatus()
      .then((data) => setGithubAppUrl(data.config.githubAppUrl || FALLBACK_GITHUB_APP_URL))
      .catch(() => {});
  }, []);
  ...
  async function handleGithubLogin() {
    try {
      ...
      window.open(data.url, 'github-oauth', 'width=800,height=700');
    } catch {}
  }
  ...
  async function handleInstallComplete() {
    ...
    try {
      ...
      if (!installation) {
        setInstallHint('Install the SYNTARO GitHub App first, then click Continue again.');
        setInstalling(false);
        return;
      }
      ...
      const result = await onboarding.completeStep('github-install', payload);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-gray-200 bg-gray-50 p-6">
        ...
        <p className="mt-1 text-sm text-gray-500">Click the button below to sign in with your GitHub account.</p>
        {!githubToken ? (
          <button type="button" onClick={handleGithubLogin} className="btn-primary mt-4 inline-flex items-center gap-2">
      ...
      <div className="rounded-lg border border-gray-200 bg-gray-50 p-6">
        ...
        <button type="button" onClick={() => window.open(githubAppUrl, '_blank')} className="btn-secondary mt-4">
          Install SYNTARO App
        </button>
        {installHint && <p className="mt-4 text-sm font-medium text-amber-600">{installHint}</p>}
      </div>
      ...
      <div className="flex items-center justify-between pt-4">
        ...
        <button type="button" onClick={handleInstallComplete} disabled={installing} className="btn-primary">
