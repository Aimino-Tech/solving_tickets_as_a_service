...
export default function IncidentDetail() {
  ...
  return (
    <div className="space-y-6">
      <Link to="/incidents" className="inline-flex min-h-[44px] items-center gap-2 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
        <ArrowLeft size={16} />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
          ...
          <div className="mt-3 space-y-3">
            ...
            <p className={`rounded-lg px-3 py-2 text-sm ${gatePassed ? 'bg-green-50 dark:bg-green-900/40 text-green-800 dark:text-green-200' : 'bg-amber-50 dark:bg-amber-900/40 text-amber-800 dark:text-amber-200'}`}>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{t('incidents.detail.metadata')}</h3>
          <dl className="mt-3 space-y-2 text-sm">
            <Row label={t('incidents.detail.firstSeen')} value={incident.firstSeenAt ? formatDateTime(incident.firstSeenAt) : '\u2014'} />
            <Row label={t('incidents.detail.lastSeen')} value={incident.lastSeenAt ? formatDateTime(incident.lastSeenAt) : '\u2014'} />
            <Row label={t('incidents.detail.dispatchedAt')} value={incident.dispatchedAt ? formatDateTime(incident.dispatchedAt) : '\u2014'} />
            <Row label={t('incidents.detail.resolvedAt')} value={incident.resolvedAt ? formatDateTime(incident.resolvedAt) : '\u2014'} />
            <Row label="Trace ID" value={incident.traceId ? <code className="font-mono text-xs">{incident.traceId}</code> : '\u2014'} />
            {incident.repos.length > 0 && (
              <Row label={t('incidents.detail.repos')} value={incident.repos.join(', ')} />
            )}
        ...
        <ol className="mt-4 space-y-0">
          {steps.map((step, idx) => (
            <li key={`${step.key}-${idx}`} className="relative flex gap-4 pb-6 last:pb-0">
              ...
              <div>
                <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{t(`incidents.detail.step.${step.key}`)}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{step.time ? formatDateTime(step.time) : '\u2014'}</p>
              </div>
            </li>
          ))}
        </ol>
      ...
      {incident.prs.length > 0 && (
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
          ...
          <ul className="mt-3 space-y-2">
            {incident.prs.map((pr, idx) => (
              <li key={`${pr.repo}-${idx}`} className="flex flex-wrap items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-700 px-3 py-2">
      ...
      {stats && (
        <div className="flex flex-wrap gap-4 text-sm text-gray-500 dark:text-gray-400">
          ...
          <span>
            {t('incidents.statsMttr')}:{' '}
            {stats.mttrSeconds != null ? formatDurationShort(stats.mttrSeconds) : '\u2014'}
          </span>
        </div>
      )}
    </div>
