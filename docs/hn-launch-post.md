# SYNTARO HN Launch Post — Ready to Publish
...
4. **Full CI pass**: SYNTARO runs your existing tests and quality gates before opening the PR. If the fix breaks something, it revises and tries again.
...
**Plan-first architecture**: This is the key differentiator. Most AI coding tools generate code immediately — they're optimized for speed. SYNTARO is optimized for correctness. It produces a design doc before it writes code. This earns trust the same way a senior engineer earns trust: by showing their thinking before they implement.

**Full-repo context**: SYNTARO doesn't just look at the file mentioned in the issue. It reads the entire repository structure, understands the dependency graph, and traces through the code path from entry point to the bug. This matters because most bugs span multiple files and modules.
...
**Complementary to existing tools**: SYNTARO is not a replacement for Cursor, Copilot, or Claude Code. Those are great at writing code when you know what you want. SYNTARO is the architect that figures out what needs to happen. They're the typists; SYNTARO is the senior engineer that plans the work.
...
3. SYNTARO posts a plan in the issue comments
...
AGPL v3 licensed. You can self-host SYNTARO for free with unlimited fixes. Or use our cloud tier for zero-config setup.

### Call to Action

Try it: install SYNTARO from GitHub Marketplace, label an issue with `syntaro:fix`, and see what it plans.

Feedback is especially welcome on:
- What kind of issues SYNTARO handles well (and where it falls short)
...
> I built SYNTARO because I was tired of context-switching out of deep work to fix bugs that should take 10 minutes but end up taking 30+ because you need to understand the full codebase first.
>
...
>
> The plan-first approach is the main thing I want feedback on. When you label an issue, SYNTARO posts a detailed plan first — you can approve it or let it auto-approve. Does this feel like the right workflow?
>
...
SYNTARO doesn't just generate code and push it. It:
...
4. The PR is opened — not merged. A human reviews it. This is exactly how you'd review a PR from a junior developer, and SYNTARO is designed for the same workflow.

### "How is this different from Copilot/Cursor/Claude Code?"
Great question. Those tools work in the IDE — you're already coding, and they help you write code faster. SYNTARO works on issues. You don't need to open your IDE. It's designed for the async workflow: label an issue, get a PR. They're complementary — use Copilot while writing code, use SYNTARO to clear your backlog.

### "LLM costs will eat us alive"
SYNTARO's median cost per fix is $3.80. At the Pro tier (500 fixes/month), that's $1,900 worth of LLM compute included in a $99/month subscription. The Team tier brings it to $2.10/fix. For self-hosted, you provide your own API keys and pay exactly what it costs.

### "Security / Trust: SYNTARO reads my entire codebase?"
SYNTARO reads the code to understand context for fixes. Data is encrypted in transit and at rest. Code snippets are retained for 90 days for fix generation, then deleted. Self-hosted option: everything stays on your infrastructure, nothing leaves your network. Cloud tier: EU-based hosting with GDPR compliance.
...
1. Full-repo context means SYNTARO understands the codebase before writing a fix
2. Plan-first architecture catches wrong approaches before code is written
3. Self-healing: if tests fail, SYNTARO revises and retries
4. We filter out issues that need human judgment (architectural decisions, new features)

### "What if SYNTARO introduces a subtle bug that tests don't catch?"
This is the most honest concern. AI-generated code, like human-written code, can contain bugs that tests don't catch. That's why SYNTARO doesn't merge — it opens a PR. The same review process you'd use for any code contribution applies. Think of SYNTARO as a junior engineer that writes good first drafts — a senior engineer still reviews the work.
