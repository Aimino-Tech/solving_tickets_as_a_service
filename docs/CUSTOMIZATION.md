...
> **How to adapt STAS to your workflow.**
...
By default, STAS reacts to the `stas:fix` label. You can change this to any label name:
...
STAS_LABEL=🤖:fix
# or
STAS_LABEL=bot-fix-this
```

### Via OpenCode Plugin

```bash
bash plugin/tools/stas-config.sh set STAS_LABEL=ai:fix
```
...
Only labels that exactly match `STAS_LABEL` trigger a fix run. This prevents conflicts with other bots.

---

## Changing the Model

STAS uses two separate models for different phases:
...
If the primary model fails (timeout, rate limit, error), STAS tries fallbacks:
...
For the main OpenCode agent, tools are configured in OpenCode Serve, not in STAS. OpenCode has its own tool system that includes:
...
export function buildPRBody(params: {
  ...
  return [
    ...
    '---',
    '',
    '_This PR was automatically generated by [STAS](https://github.com/tamnguyen08/solving_tickets_as_a_service)._',
  ].join('\n');
}
```
...
- Issue status comments: `> 🤖 **STAS:** Investigating...`
- PR descriptions: `_This PR was automatically generated by [STAS](...)_`
- Commit author: `STAS Bot <stas-bot@users.noreply.github.com>`
...
E2B_TEMPLATE_ID=stas-default            # E2B template ID
```
...
```bash
STAS_MAX_CONCURRENT=3          # Max concurrent fixes per repo
...
STAS supports multiple webhook sources beyond GitHub:
...
When tickets arrive from Linear or Jira, STAS needs to know which GitHub repo to use:
...
```bash
STAS_RATE_LIMIT_WINDOW_MS=60000    # Window (default: 1 min)
...
```bash
STAS_DEFAULT_TIER=free          # Options: free, pro, enterprise
STAS_MONTHLY_QUOTA_ENABLED=true # Enable/disable quota enforcement
```
...
STAS_CONCURRENCY_OVERRIDES=my-org/my-repo=5,my-org/important-repo=10
```
...
| `BOT_NAME` | `STAS` | Bot display name |
...
| `STAS_DEFAULT_TIER` | `free` | Default billing tier |
...
| `E2B_TEMPLATE_ID` | `stas-default` | E2B template |
