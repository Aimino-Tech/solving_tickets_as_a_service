...
- **Harassment** — Any form of harassment, including but not limited to: offensive comments related to protected characteristics, deliberate intimidation, stalking, following, unwanted photography or recording, inappropriate physical contact, and unwelcome sexual attention.
- **Discrimination** — Discriminatory jokes, language, or behavior targeting any group or individual.
- **Trolling and insults** — Deliberately inflammatory, insulting, or derogatory comments, personal attacks, or political agitation.
...
Community moderators are responsible for clarifying and enforcing our standards. Moderators have the authority to:

- Remove, edit, or reject messages, comments, commits, code, wiki edits, issues, and other contributions that do not align with this Code of Conduct.
- Issue warnings for minor or first-time violations.
- Temporarily mute users in channels for repeat minor violations.
- Temporarily ban users from the workspace for serious or repeat violations.
- Permanently ban users for severe or persistent violations.

Moderators are expected to:

- Act fairly and consistently — apply the same standards to all members regardless of status or tenure.
- Act promptly — address reported violations within 24 hours.
- Document actions — log all moderation actions in #mod-log with clear reasoning.
...
- Final authority on Code of Conduct interpretations and appeals.
- Oversee moderator elections and removals.
- Review moderation logs weekly for consistency.
- Handle violations involving moderators themselves.
- Communicate major enforcement decisions to the community.
...
- **Your contact info** — Your Slack handle or email address so we can follow up.
- **Date and time** — When the incident occurred (including timezone).
- **Location** — Which channel, thread, GitHub issue/PR, or other community space.
- **Description** — What happened, including any relevant context. Be as specific as possible.
- **Witnesses** — Who else was present or saw the incident.
- **Evidence** — Screenshots, links, or message permalinks.
- **Impact** — How the incident affected you or others.
- **Resolution request** — What outcome you are seeking (e.g., warning, removal of content, ban).

### Confidentiality

All reports are treated as confidential. We will not share your identity with the reported individual without your explicit consent. Reports are shared only with the minimum number of moderators and maintainers necessary to investigate and resolve the issue.
...
**When applied**: First or minor violation — a mistake, a misunderstanding, or an isolated incident that does not cause significant harm.

**Action**: A moderator sends a private DM to the individual explaining:
- Which behavior violated the Code of Conduct
- Which standard was violated (with a link to this document)
- What corrective action is expected (e.g., remove a post, apologize)
- That this is a formal warning and further violations will result in escalated consequences
...
**When applied**:
- Repeat violations after a warning (2+ warnings in 90 days)
- A single serious violation that causes harm but is not egregious (e.g., sustained harassment, posting someone's private information)
- Violation of a temporary mute or channel restriction

**Action**: The individual is removed from the Slack workspace for a defined period:
- **First temporary ban**: 7 days
- **Second temporary ban**: 30 days
- **Third temporary ban**: reviewed for permanent ban

The moderator team notifies the individual via email with:
...
```
              ...
              ▼                ▼                ▼
    ┌──────────────────┐ ┌──────────┐ ┌────────────────┐
    │ Minor / First    │ │ Serious  │ │ Egregious      │
    │ Violation        │ │ Violation│ │ Violation      │
    └────────┬─────────┘ └────┬─────┘ └───────┬────────┘
             │                │                │
             ▼                ▼                ▼
    ┌──────────────────┐ ┌──────────┐ ┌────────────────┐
    │ Step 1: Warning  │ │ Step 2:  │ │ Step 3:        │
    │                  │ │ Temp Ban │ │ Permanent Ban  │
    │ + DM notification│ │ (7-30d)  │ │                │
    │ + Logged in      │ │ + Email  │ │ + Email        │
    │   #mod-log       │ │   notice │ │   notice       │
    │ + Expires 90d    │ │ + Return │ │ + Blocked from │
    └──────────────────┘ │   conds  │ │   all spaces   │
                         └──────────┘ └────────────────┘
```
