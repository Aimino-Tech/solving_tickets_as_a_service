# SYNTARO Growth Initiative: Phase 2, Activation Funnel Optimization

> **Status**: Execution plan (strategy document)
> **Owner**: SYNTARO Growth Initiative (AIM-4396)
...
Phase 2 exists to make the path from *installation* to *first merged fix* as short and self-serve as possible. New users decide whether SYNTARO works within their first few issues; a fast, visible first fix is the strongest activation signal available. The parent initiative sets two primary metrics; the supporting metrics below keep the funnel honest.
...
|---|---|---|---|
| 1. Welcome | On `installation.created`, SYNTARO creates a welcome issue that explains how to trigger a fix | Webhook handler | Welcome issue labeled `syntaro:fix` on every repo of the installation |
| 2. Trigger | The user labels an issue (or a pre-seeded welcome issue) `syntaro:fix` | User / welcome issue | Issue queued on the `syntaro.issues.fix` queue |
...
| 4. Review & merge | CI passes → SYNTARO requests review from collaborators and (when enabled) joins the merge queue | Quality gate | Reviewed, merged PR |
...
**Workstream 1: welcome issue auto-creation (done).** `createWelcomeIssue` (`src/webhooks/github.ts:111-173`) runs on the `installation.created` webhook event (`github.ts:209-313`, per-repo loop at :301-309). It ensures the trigger label exists (`config.syntaro.label`, color `0366d6`, description "Trigger a SYNTARO AI fix for this issue"), then opens an issue titled "Welcome to SYNTARO — let's fix your first issue" whose body walks through what happens next, the `syntaro:fix` label, and a demo issue, finishing with a "Powered by Syntaro" footer. The function is best-effort: failures are logged and surfaced but never fail the installation event.
...
**Dispatch timeout (done).** `QUEUE_MSG_TTL_MS` defaults to `30_000` (`src/config.ts:47`), cutting the previous 120-second dispatch timeout to 30 seconds for quick-turnaround issues. The value flows to `config.queue.msgTtlMs` (`src/config.ts:419`) and is applied to the `syntaro.issues.fix` queue (`src/queue/rabbitmq.ts:144`); other queues keep a 10-minute TTL.
...
- **Queue latency**: time spent queued on `syntaro.issues.fix`; the 30s TTL is the hard bound.
...
- Phase roadmap: `docs/syntaro/growth-initiative-phase-6-one-million-users.md` (Phase 2 of that roadmap consumes this funnel)
- Content roadmap: `docs/syntaro/growth-initiative-phase-4-content-engine.md`
