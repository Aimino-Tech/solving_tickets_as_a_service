...
The 3-repo integration suite (SYNTARO ↔ governance ↔ OpenSymphony) composes SYNTARO,
...
upstream was absent. The full `webhook → SYNTARO → governance → OpenSymphony → 202`
...
1. **Add an `opensymphony` service to `tests/integration/docker-compose.yml`**,
   ...
   is a dependency only of the *stack*, not of SYNTARO — SYNTARO's `depends_on` is
...
- No SYNTARO regression risk: the SYNTARO webhook path does not depend on OpenSymphony
...
1. **SYNTARO container crashed at boot** — `src/utils/logger.ts` loads `pino-pretty`
   whenever `NODE_ENV != "production"` (the stack runs `NODE_ENV=test`), but
   `tests/integration/Dockerfile.syntaro` ran `npm prune --production` which removed
   the devDependency. Fix: reinstall `pino-pretty@^13.1.3` (`--no-save`) in the
   runtime stage.
2. **SYNTARO `/health` returned 503** — tsc emits only `.ts → .js`, so the migration
   ...
   `wait-for-health.sh` before waiting for SYNTARO.
...
## SYNTARO service runtime tweaks (integration image)

The integration stack runs no real opencode server, so two SYNTARO-side settings were
needed for the stack to become healthy:

- `SYNTARO_AI_MODE: static` in the compose — `opencodeHealth` reports `healthy` in
  static mode, so SYNTARO `/health` returns 200 (it otherwise 503s on the degraded
