...
**Title option C:**
> Show HN: STAS — I built an AI that reads your repo, plans the fix, then opens a PR
...
STAS is different. It's an AI senior architect that reads your entire repo, produces a detailed natural-language plan, and then writes the fix — all from a single GitHub issue label.

### How it works

1. Install the GitHub app or action
2. Label an issue with `stas:fix`
3. Within 15 seconds, STAS posts a plan to the issue explaining what it will change and why
4. You review the plan, approve it (or ask for changes)
5. Within 45 seconds, STAS opens a PR with the fix

The key insight: **plan first, code second.** This is the opposite of every other AI coding tool. Instead of generating code and hoping it works, STAS shows its reasoning before writing a single line. You get to review the architecture, not just the diff.
...
STAS is neither. It's **asynchronous by design** — you label an issue and come back to a PR. It reads the entire repo (not just open tabs, not just a single file) and produces a plan that a senior engineer would be proud to review.

### What's under the hood

- Full-repo code graph analysis — STAS builds a dependency graph of your codebase to understand how changes propagate
...
> ⚠️ *Demo GIF to be captured at launch — install STAS on a test repo, label an issue with `stas:fix`, and screen-record the plan → PR flow. Target: under 2 minutes, under 5 MB.*
...
Try it: install from GitHub Marketplace, label any issue with `stas:fix`, and watch it plan. Feedback welcome — I'll be in the comments all day.

https://github.com/marketplace/stas
...
Claude Code is a terminal tool that requires you to be present. STAS runs as a GitHub bot — you label an issue, walk away, and come back to a PR. It's architected for async work, not pair programming.

**Q: Can I trust AI-generated code?**
You review the plan before any code is written. Every PR includes tests. And because STAS produces a plan first, you catch architectural issues early — before there's code to review.

**Q: What about security?**
STAS reads your repo to understand the codebase. Code is processed in-memory and not stored after the fix is generated. GitHub tokens are encrypted at rest. Self-hosted option available for air-gapped environments.
