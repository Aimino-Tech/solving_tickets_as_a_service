export const DEFAULT_BOT_NAME = 'STAS';
function botSignature(botName) {
    return `> — ${botName} 🤖`;
}
export function highConfidenceIssueComment(prNumber, result, botName) {
...
export function draftIssueComment(prNumber, result, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function lowConfidenceComment(result, testOutput, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function noFixComment(result, relevantPRs, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    const lines = [`### ❌ Could Not Fix`, '', result.noFixReason || result.summary, ''];
    if (relevantPRs && relevantPRs.length > 0) {
        lines.push('**Related pull requests**:', '', ...relevantPRs.map((pr) => `- [${pr.title}](${pr.url}) — ${pr.state}`), '');
    }
    lines.push('This issue may need manual investigation. Contributions are welcome!', '', sig);
    return lines.join('\n');
}
export function noResultComment(botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
        `### 🤔 Unexpected Result`,
        '',
...
export function investigationComment(summary, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function alreadyFixedComment(result, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function errorComment(message, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
        `### ❌ Error`,
        '',
...
export function featureSkipComment(botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function questionSkipComment(botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
        `### ❓ Question Detected`,
        '',
...
export function timeoutComment(phase, timeoutMs, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
...
export function retryComment(attempt, model, error, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function modelFallbackComment(model, previousError, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function queueRetryComment(attempt, maxRetries, error, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function deadLetterComment(error, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function phantomIssueComment(missingFiles, skipReason, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function ciFailureComment(prNumber, failedChecks, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    return [
...
export function regressionBlockComment(result, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
...
export function verificationWarningComment(result, botName) {
    const sig = botSignature(botName ?? DEFAULT_BOT_NAME);
    const ver = result.verification;
    if (!ver)
        return '';
    const lines = [`### ⚠️ Verification Warnings`, ''];
    if (ver.regressionTestPassedOnOriginal === false) {
        lines.push('- The regression test does **not fail** when run against the original code.', '  It may not properly validate the bug fix.');
    }
    if (ver.regressionTestPassedOnFix === false) {
        lines.push('- The regression test does **not pass** on the fixed code.', '  The test may need adjustment.');
    }
    if (ver.regressionTestCreated === false) {
        lines.push('- No new regression test file was detected.', '  Manual verification is recommended.');
    }
    if (ver.unverified) {
        lines.push('- No test suite was detected. Verification was skipped.', '  Manual testing is recommended before merging.');
    }
