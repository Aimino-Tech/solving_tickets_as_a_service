#!/usr/bin/env python3
"""XGBoost model runner for tabular-mcp benchmark.

Usage: echo <csv_data> | python3 run_xgboost.py --target <col> --format json
"""
import argparse
import json
import sys
...
def main():
    parser = argparse.ArgumentParser(description="XGBoost benchmark runner")
    ...
    if not HAS_XGB:
        # Fallback: use sklearn RandomForest
        ...
        if is_regression:
            ...
            model = xgb.XGBClassifier(n_estimators=100, random_state=42, verbosity=0, use_label_encoder=False)
        model.fit(X_train, y_train)
